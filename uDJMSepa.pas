...
begin
   ...
   for i := 1 to Length(sOut) do begin
      if not(Ord(sOut[i]) in [65..90, 97..122, 48..57, 47, 45, 63, 58, 40, 41, 46, 44, 39, 43, 32]) then sOut[i] := ' ';
   end;
   ...
   if (iMaxLen >= 0) and (Length(sOut) > iMaxLen) then sOut := Copy(sOut, 1, iMaxLen);
   Result := sOut;
end;
